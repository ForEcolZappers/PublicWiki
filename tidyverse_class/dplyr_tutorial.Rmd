...
Can you use `filter()` to inspect flights from the first day of the year? (hint: use `month == ` and `day == `)?
...
How would you create a new variable that tells us the average speed during the flight (`speed`)
...
The verb `summarize()` colamples a data frame to a single row. For examples, we can use it to calculate the average delay.
