...
`pivot_longer()` also has a values_ptypes argument that controls the type of the values_to column. You specify `values_ptypes` in the same way as `names_ptypes`.
...
Now, can you return the tidy Tibble to its original form using `pivot_wider()`?
...
```
Is this data tidy?
What variables can you identify? (there should be four)
